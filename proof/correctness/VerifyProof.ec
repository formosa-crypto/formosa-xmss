...
while (
  ...
  0 <= to_uint idx_leaf{1} < 2^XMSS_FULL_HEIGHT /\
   ...
   sm_ptr{1} = ptr_sm /\
    ...
    ((j{2} = 1) => (to_list root{1} = NBytes.val _M'{2})) 
); last first.
...
seq 0 6: (
  #pre /\
  idx_sig1{2} = idx_leaf{2} /\
  sig_ots1{2} = sig_ots{2} /\
  auth1{2} = auth{2} /\
  M0{2} = _M'{2} /\
  NBytes.val _seed0{2} = to_list root{1} /\
  _seed0{2} = node{2} /\
  address1{2} = address{2}
). 
auto => /> &1 &2 /#.
