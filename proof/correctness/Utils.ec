...
rewrite wordP => j?.
rewrite xorwE.
have E0: W32.one.[0] by rewrite /W32.one bits2wE initiE //= /int2bs nth_mkseq.
...
+ by move => y /mapP[] z [] ? ->; rewrite NBytes.valP.
...
(*                              HA                                                    *)
(* ------------------------------------------------------------------------------- *)
suff -> : (to_uint w1 %/ 2 ^ (8 * (32 - (j + 1)))) = 0 by done.
apply: pdiv_small; split => [ | ? ]; first by [].
apply: (StdOrder.IntOrder.ltr_le_trans (2 ^ 32 - 1)); first by [].
apply: (lez_trans (2 ^ 32)); first by [].
apply: StdOrder.IntOrder.ler_weexpn2l; first by [].
smt().
