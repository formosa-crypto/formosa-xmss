...
lemma exists_nonneg_succ_height_node (l r : 'a bintree) : exists (n : int), 0 <= n /\ height (Node l r) = 1 + n.
...
*)
op perfectOf (bt : 'a bintree) (n : int) =
  with bt = Leaf _ => n = 0
  with bt = Node l r => perfectOf l (n - 1) /\ perfectOf r (n - 1).

lemma height_eq_of_perfectOf (bt : 'a bintree) :
  forall (n : int), perfectOf bt n => height bt = n.
proof.
  elim : bt => // /= ? ? Hl Hr ? [Hl2 Hr2].
  by rewrite (Hl _ Hl2) // (Hr _ Hr2).
qed.

lemma nonneg_of_perfectOf (bt : 'a bintree) (n : int) :
  perfectOf bt n => 0 <= n.
proof.
  by move => H; rewrite -(height_eq_of_perfectOf bt n H); exact (height_nonneg bt).
qed.

lemma one_le_of_perfectOf_pred (bt : 'a bintree) (n : int) :
  perfectOf bt (n - 1) => 1 <= n.
proof. smt(nonneg_of_perfectOf). qed.

lemma perfectOf_iff_ite_exists_leaf_exists_node (bt : 'a bintree) (n : int) :
  perfectOf bt n <=> if n = 0 then exists x, bt = Leaf x else
  (exists t1 t2, bt = Node t1 t2 /\ perfectOf t1 (n - 1) /\ perfectOf t2 (n - 1)).
proof.
  case : bt => [x /# | l r /=].
  smt (one_le_of_perfectOf_pred).
qed.

(*
  Determines whether a binary tree is perfect.
*)
op perfect (bt : 'a bintree) = perfectOf bt (height bt).
...
proof.
  ...
  split.
  - move => [Hl Hr].
    by rewrite (height_eq_of_perfectOf _ _ Hl)
    (height_eq_of_perfectOf _ _ Hr) Hl Hr.
  - move => [Hlr [Hl Hr]] /=.
...
lemma perfect_iff_ite_exists_leaf_exists_node (bt : 'a bintree) :
  perfect bt <=> if height bt = 0 then exists x, bt = Leaf x else
  (exists t1 t2, bt = Node t1 t2 /\ height t1 = height t2 /\ perfect t1 /\ perfect t2).
proof.
 case : bt => [x | l r].
  - by rewrite perfect_leaf /=; exists x.
  - by rewrite perfect_node /=; smt(height_nonneg).
qed.
