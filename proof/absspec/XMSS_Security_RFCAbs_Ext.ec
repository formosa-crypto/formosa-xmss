  ...
  op FLXMSSTW.SA.WTW.prf_sk <-
    (fun (ss : nbytes) (psad : nbytes * FLXMSSTW.SA.adrs) =>
     DigestBlock.insubd (BytesToBits
                         (NBytes.val
                          (prf_keygen ss (psad.`1, (idxs2adrs (FLXMSSTW.SA.HAX.Adrs.val psad.`2))))))),
  op FLXMSSTW.SA.WTW.thfc <-
...
op ads2adr (ad : SA.adrs) : Address.adrs = idxs2adrs (HAX.Adrs.val ad).
...
op wots_pk_val(ss ps : Params.nbytes, ad : SA.adrs) : dgstblocklenlist =
   pkWOTS_from_skWOTS (gen_skWOTS ss ps ad) ps ad.

op leafnode_from_idx(ss ps : Params.nbytes, ad : SA.adrs, lidx : int) : dgstblock =
 let pk = wots_pk_val ss ps (set_kpidx (set_typeidx ad 0) lidx) in
...
lemma Eqv_WOTS_pkgen (ad : SA.adrs) (ss ps : seed) :
  phoare[WOTS.pkGen : arg = (ss, ps, ads2adr ad) ==>
     LenNBytes.insubd (map NBytes.insubd (chunk n (BitsToBytes (flatten (map DigestBlock.val (DBLL.val
       (wots_pk_val ss ps ad))))))) = res] = 1%r.
admitted.
...
trh ps (set_thtbidx (set_typeidx ad 2) (lidx %/ 2 ^ ((nth witness si (offset - 1)).`2 + 1)) ((nth witness si (offset - 1)).`2))
...
(* FD + WR *)
equiv kg_eq (O <: Top.XMSS_RFC_Abs.Oracle) : XMSS_TW(O).keygen ~ XMSS_RFC_Abs(O).kg :
  ={arg, glob O} ==> pkrel res{1}.`1 res{2}.`2 /\ skrel res{1}.`2 res{2}.`1.
proof.
...
while (size leafl0{1} = i{2} /\ 0 <= i{2} <= 2^h /\ t{2} = h /\ s{2} = 0
    /\ ps{1} = pub_seed1{2} /\ ad{1} = ad0{1} /\ ps1{1} = ps{1} /\ ss1{1} =sk_seed1{2} /\ ss1{1} = ss{1}
...
+ auto => /> &1; do split.
  ...
  + move => leafs1 hs1 o1 st2 10? Hl 2? H.
...
+ seq 2 3 : (#pre /\   pk0{2} = LenNBytes.insubd
  (map NBytes.insubd
     ...
     pkWOTS{1} =   wots_pk_val ss1{1} ps1{1} (set_kpidx (set_typeidx ad0{1} 0) (size leafl0{1})) (size leafl0{1})).
  + conseq />;1: smt(). 
