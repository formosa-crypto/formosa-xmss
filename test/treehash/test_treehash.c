...
void test_condition(const xmss_params *p) {
    ...
    // Caso 1: Offset >= 2 e verdadeiro => a segunda parte da conjuncao e avaliada (provavelmente e
    // false)
    ...
    // Caso 2: Offset >= 2 e verdadeiro => a segunda parte da conjuncao e avaliada (vai ser
    // verdadeira)
    for (int i = 0; i < TESTS; i++) {
        if (debug) {
    ...
    for (int i = 0; i < TESTS; i++) {
        ...
        assert(offset < 2);
        /* assert(offset >= 0); */ /* warning: comparison of unsigned expression in ‘>= 0’ is always
                                      true */
...
#define XMSS_MLEN 32
    ...
    for (int i = 0; i < TESTS; i++) {
        ...
        assert(memcmp(sk_ref, sk_test, p->sk_bytes) ==
               0);  // Make sure the keypair is the same (we only need the sk)
