...
// extern void gen_leaf_wots_jazz(uint8_t *leaf, uint32_t ltree_addr[8], uint32_t ots_addr[8],
...
static xmss_params init_params(const char *impl) {
    if (!impl) {
        fprintf(stderr, "Implementation name is NULL\n");
        exit(-1);
    }
...
void test_ltree(void) {
    ...
    xmss_params p = init_params(xstr(IMPL));
...
#define XMSS_MLEN 32
    ...
    for (int i = 0; i < 100; i++) {
        ...
        int res_jasmin =
            xmssmt_core_sign_open_jazz(mout_jasmin, &mlen_jasmin, sm, smlen, pk + XMSS_OID_LEN);
...
void test_sign_open_xmssmt(void) {
    bool debug = true;
...
#define XMSS_MLEN 32
    ...
    for (int i = 0; i < 100; i++) {
        ...
        xmss_sign(sk, sm, &smlen, m, XMSS_MLEN);
        assert(smlen == p.sig_bytes + XMSS_MLEN);
        int res = xmss_sign_open_jazz(mout, &mlen, sm, smlen, pk);
...
#undef XMSS_MLEN
    ...
    puts("[xmss_commons - sign open] OK");
}
