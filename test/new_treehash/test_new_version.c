...
#define XMSS_MLEN 20
    ...
    for (int i = 0; i < TESTS; i++) {
        ...
        if (memcmp(sm_new, sm_ref, p->sig_bytes + XMSS_MLEN) != 0) {
            ...
            // for the multi tree variant, it is WOTS SIG || AUTH PATH || WOTS SIG || AUTH PATH ||
            // WOTS SIG || AUTH PATH
            // ......
            if (p->pk_bytes == 1) {
                // wots signature :
                ...
                for (unsigned int k = 0; k < p->wots_len; k++) {
                    // puts("==============================================================================================");
                    // printf("Testing k=%d\n", k);
                    // fprint_str_u8("wots_sig.txt", "sm", sm_new + p->index_bytes + p->n + (k *
                    // p->n), p->n); fprint_str_u8("wots_sig.txt", "sm", sm_ref + p->index_bytes +
                    // p->n + (k * p->n), p->n);
                ...
                assert(memcmp(sm_new + p->index_bytes + p->n, sm_ref + p->index_bytes + p->n,
                              p->wots_sig_bytes) == 0);
                puts("Wots Sig: OK");
                puts(
                    "=============================================================================="
                ...
                printf("Auth Path Preffix length: %ld\n",
                       longestCommonPrefixSize(sm_new + p->index_bytes + p->n + p->wots_sig_bytes,
                                               sm_ref + p->index_bytes + p->n + p->wots_sig_bytes,
                                               p->tree_height * p->n));
...
int main() {
    ...
    // char *impls[] = {"XMSS-SHA2_10_256", "XMSS-SHA2_10_192", "XMSSMT-SHA2_20/2_256",
