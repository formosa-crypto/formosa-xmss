...
extern void treehash_jazz(uint8_t *root, const uint8_t *sk_seed, const uint8_t *pub_seed,
                          uint32_t start_index, uint32_t target_height,
                          const uint32_t *subtree_addr);

extern void build_auth_path_jazz(uint8_t *auth_path, const uint8_t *sk_seed,
                                 const uint8_t *pub_seed, uint32_t i, uint32_t *addr);
...
void treehash_new(const xmss_params *params, unsigned char *root, const unsigned char *sk_seed,
                  ...
                  uint32_t start_index, /* leaf_idx in the old impl */
    ...
    while (i < (uint32_t)(1 << target_height)) {
        ...
        while (offset >= 2 && heights[offset - 1] == heights[offset - 2]) {
            ...
            thash_h(params, stack + (offset - 2) * params->n, stack + (offset - 2) * params->n,
                    pub_seed, node_addr);
            offset--;
...
// treesig
void treesig(const xmss_params *params,
             unsigned char *sig /* reduced signature =  wots signature || auth path */,
...
// sign
int xmssmt_core_sign_new(const xmss_params *params, unsigned char *sk, unsigned char *sm,
    ...
    *smlen = params->sig_bytes + mlen;
    ...
    idx_leaf = (idx & ((1 << params->tree_height) -
                       1)); /* (h - h / d) least significant bits of idx_sig */
    ...
    for (unsigned int i = 1; i < params->d; i++) {
        ...
        idx_tree =
            idx_tree >> params->tree_height; /* (h - h / d) most significant bits of idx_sig */
        idx_leaf = (idx_tree & ((1 << params->tree_height) -
                                1)); /* (h - h / d) least significant bits of idx_sig */
...
/*
 * Esta implementacao (assumindo que k e implementado como um inline int e, portanto, os if sao
 * resolvidos em compile time) e' ligeiramente mais facil de provar.
 *
 * Tambem usa menos stack
 *
 * Alem disso, na primeira iteracao, nao precisamos de copiar leaf para nodes0 e depois copiar para
