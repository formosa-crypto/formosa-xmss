...
extern void gen_chain_jazz(uint8_t *, uint32_t *, const uint8_t *, uint32_t, uint32_t,
                           const uint8_t *);
#endif
 ...
 */
static void base_w(const xmss_params *params, int *output, const int out_len,
...
#ifdef TEST_GEN_CHAIN_INPLACE
        gen_chain_inplace_jazz(pk + i * params->n, addr, 0, params->wots_w - 1,
                               pub_seed);  // start = 0
#else
        gen_chain(params, pk + i * params->n, pk + i * params->n, 0, params->wots_w - 1, pub_seed,
                  addr);
#endif
    }
}
...
#ifdef TEST_GEN_CHAIN
        gen_chain_jazz(pk + i * params->n, addr, sig + i * params->n, lengths[i],
                       params->wots_w - 1 - lengths[i], pub_seed);
#else
        gen_chain(params, pk + i * params->n, sig + i * params->n, lengths[i],
                  params->wots_w - 1 - lengths[i], pub_seed, addr);
#endif
    }
}
