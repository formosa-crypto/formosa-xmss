...
extern void compute_root_jazz(unsigned char *root, uint32_t addr[8], const unsigned char *leaf,
                              unsigned long leaf_idx, const unsigned char *auth_path,
                              const unsigned char *pub_seed);
#endif

#ifdef TEST_HASH_MESSAGE
extern void hash_message_jazz(uint8_t *, const uint8_t *, const uint8_t *, uint64_t,
...
void compute_root(const xmss_params *params, unsigned char *root, const unsigned char *leaf,
    ...
    for (i = 0; i < params->tree_height - 1; i++) {
        ...
        if (leafidx & 1) {
#ifdef TEST_THASH_H
            // NOTE: in the jasmin impl, addr is the 2nd argument because writable ptrs must come
...
#ifdef TEST_THASH_H
    // NOTE: in the jasmin impl, addr is the 2nd argument because writable ptrs must come first in
    // the arguments
...
#ifdef TEST_LTREE
    // NOTE: The addr is before pub_seed in the jasmin impl because the writable ptrs must come
...
#ifdef TEST_COMPUTE_ROOT
        // NOTE: The addr is the 2nd argument in the jasmin impl because the writable ptrs must come
        // first in the list of arguments
