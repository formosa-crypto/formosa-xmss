...
void xmssmt_bench_verify(const xmss_params *params, uint32_t oid) {
    ...
    // Generate a valid signature to verify
    xmssmt_sign(sk, sm, (unsigned long long *)&smlen, m, MESSAGE_SIZE);


    for (size_t i = 0; i + 1 < DATA_POINTS; i++) {
        if (verbose) {
            printf("verify [%s]: %zu/%d\n", get_impl_name(buf, 256), i, DATA_POINTS);
        }

        before = cpucycles();
        
        #ifdef REF
        result = xmssmt_sign_open(m, (unsigned long long *)&mlen, sm, params->bytes + MESSAGE_SIZE, pk);
        #else
...
#ifdef MEDIAN_ONLY
    write_results(xstr(OUTPUT_FILE), "verify", median_val, 0);
#else
    uint64_t avg = average(observations, DATA_POINTS);
    write_results(xstr(OUTPUT_FILE), "verify", median_val, avg);
#endif
}
